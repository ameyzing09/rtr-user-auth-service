# Check Tenant Status
# Usage: .\scripts\check-tenants.ps1
...
Write-Host "==> Checking Tenant Provisioning Status" -ForegroundColor Cyan
Write-Host ""

$query = "SELECT status, COUNT(*) as count FROM tenants GROUP BY status;"
...
Write-Host "  ACTIVE: X" -ForegroundColor Green
Write-Host "  PENDING: X" -ForegroundColor Yellow
Write-Host "  PROVISIONING: X" -ForegroundColor Cyan
